export const PREDICT_PACKAGE =
  "0xf5ea2b3749c65d6e56507cc35388719aadb28f9cab873696a2f8687f5c785138";

export const PREDICT_OBJECT =
  "0xc8736204d12f0a7277c86388a68bf8a194b0a14c5538ad13f22cbd8e2a38028a";

export const PREDICT_REGISTRY =
  "0x43af14fed5480c20ff77e2263d5f794c35b9fab7e2212903127062f4fe2a6e64";

export const QUOTE_TYPE =
  "0xe95040085976bfd54a1a07225cd46c8a2b4e8e2b6732f140a0fc49850ba73e1a::dusdc::DUSDC";

export const PLP_TYPE = `${PREDICT_PACKAGE}::plp::PLP`;

export const PREDICT_SERVER = "https://predict-server.testnet.mystenlabs.com";

export const CLOCK_OBJECT = "0x6";

export const QUOTE_DECIMALS = 6;

export const predictTarget = (fn: string) =>
  `${PREDICT_PACKAGE}::predict::${fn}` as const;

export const managerTarget = (fn: string) =>
  `${PREDICT_PACKAGE}::predict_manager::${fn}` as const;

export const MANAGER_TYPE = `${PREDICT_PACKAGE}::predict_manager::PredictManager`;
